(function () {
  ...
  function renderImpressumPage() {
    return '<a href="#" class="back-link">' + t('backToCalculator') + '</a>' +
      ...
      '<p><strong>Urheberrecht</strong><br>Der Quellcode dieser Website ist als Open-Source-Projekt unter der <a href="https://github.com/markus-sanwald/tfw-rechner/blob/main/LICENSE" target="_blank" rel="noopener">MIT-Lizenz</a> ver&ouml;ffentlicht. Nutzung, Kopie, Ver&auml;nderung und Weiterverbreitung des Quellcodes sind im Rahmen der MIT-Lizenz ausdr&uuml;cklich gestattet. Die durch den Seitenbetreiber erstellten Inhalte (Texte, Berechnungslogik, Gestaltung) unterliegen dem deutschen Urheberrecht. Soweit Inhalte Dritter verwendet wurden, werden deren Urheberrechte beachtet. Sollten Sie auf eine Urheberrechtsverletzung aufmerksam werden, bitten wir um einen entsprechenden Hinweis.</p>' +
  ...
  function renderDatenschutzPage() {
    return '<a href="#" class="back-link">' + t('backToCalculator') + '</a>' +
      ...
      '<p>Der Betreiber dieser Website erhebt und speichert automatisch Informationen in sogenannten Server-Logfiles, die Ihr Browser automatisch an uns &uuml;bermittelt. Dies sind: Browsertyp und -version, verwendetes Betriebssystem, Referrer-URL, Hostname des zugreifenden Rechners, Uhrzeit der Serveranfrage sowie IP-Adresse.</p>' +
      '<p>Eine Zusammenf&uuml;hrung dieser Daten mit anderen Datenquellen wird nicht vorgenommen. Rechtsgrundlage ist Art. 6 Abs. 1 lit. f DSGVO (berechtigtes Interesse an einem sicheren und stabilen Betrieb der Website). Die Logfiles werden nach sp&auml;testens 14 Tagen gel&ouml;scht.</p>' +
      '<h2>Lokaler Browserspeicher (localStorage)</h2>' +
      '<p>Diese Website speichert Ihre Sprachpr&auml;ferenz (Deutsch/Englisch) im lokalen Speicher Ihres Browsers (localStorage). Dieser Eintrag verbleibt ausschlie&szlig;lich auf Ihrem Ger&auml;t und wird nicht an uns oder Dritte &uuml;bertragen. Es handelt sich um keine personenbezogenen Daten; es werden keine Nutzerprofile erstellt. Rechtsgrundlage ist Art. 6 Abs. 1 lit. f DSGVO (berechtigtes Interesse an der Benutzerfreundlichkeit). Sie k&ouml;nnen den lokalen Speicher jederzeit &uuml;ber die Einstellungen Ihres Browsers l&ouml;schen.</p>' +
  ...
  function buildCalculator() {
    ...
    verzinsungGroup.innerHTML =
      ...
      ' <span id="vz-label-rate" style="color:var(--color-text-muted);font-size:0.85rem;">(' + String(verzinsungRate).replace('.', ',') + '\u00A0%)</span>' +
      '</label>' +
    ...
    var vzLabelRate = verzinsungGroup.querySelector('#vz-label-rate');
    ...
    vzRateInput.addEventListener('input', function(e) {
      ...
      if (!isNaN(val) && val >= 0) {
        verzinsungRate = val;
        vzLabelRate.textContent = '(' + String(val).replace('.', ',') + '\u00A0%)';
        updateResult();
      }
    });
